...
const Card = (props) => (
  <div style={{ display: "inline-block", minHeight: 50, minWidth: 200, textAlign: "center", border: "1px solid #dddddd", padding: 10, marginLeft: 50}}>
    {props.title}
  </div>
)
...
class SubthemeSection extends React.Component {
  render() {
    ...
    const allRelationships = [
      ...defaultToEmpty(subtheme.relationships.articles).map(article => (<Card title={article.title} />)),
      ...defaultToEmpty(subtheme.relationships.clips).map(clip => (<Card title={clip.title} />)),
      ...defaultToEmpty(subtheme.relationships.faqs).map(faq => (<Card title={faq.title} />)),
    ]

    const description = subtheme.description ? [<div style={{minWidth: 300, padding: 10 }} dangerouslySetInnerHTML={{ __html: subtheme.description.processed }} />] : [];
    ...
    return (
      <div style={{ border: "1px solid #aaaaaa", padding: 20, marginBottom: 50 }}>
        <h3>{subtheme.name}</h3>
  ...
  render() {
    ...
    return (
      <div>
        ...
        {
          theme.description ?
            <div style={{minWidth: 300 }} dangerouslySetInnerHTML={{ __html: theme.description.processed }} /> :
            null
        }
        {
